...
def css():
    ...
    /* V195-3 THEME COLOR FIX
       기존 V107-5/V108-2/V108-3에서 해결했던 다크/라이트 모드 글자색 문제 재발 방지.
       원인: .card * {color:#0f172a!important}가 V195 검은 박스 내부까지 덮어씀.
       해결: V195 작전명령 검은 박스와 모든 자식에 흰색 !important를 후순위로 강제.
    */
    .v195-command-dark,
    .v195-command-dark *,
    .v195-command-dark div,
    .v195-command-dark span,
    .v195-command-dark b,
    .v195-command-dark strong,
    .v195-command-dark p {
...
def _v174_apply_live_quote_to_record(rec):
    """V174: 일봉 차트 기준 레코드에 KIS 실시간 현재가를 덮어 추천 필터를 다시 확인합니다.
    - 5만원 이하
    - 현재가 > 60일선
    - 전저점 유지
    실시간 조회 실패 시 기존 일봉 종가 기준을 유지합니다.
    """
    try:
        ...
        if q and q.get("ok") and q.get("price"):
            ...
            rec["live_change_rate"] = q.get("change_rate")
            rec["live_volume"] = q.get("volume")
            rec["live_amount"] = q.get("amount")
            rec["live_src"] = q.get("src", "KIS")
            rec["real_time_checked"] = True
            # 추천 판단의 현재가는 실시간 가격을 우선합니다.
            rec["close"] = live_price
            rec["price_preferred"] = bool(live_price <= float(rec.get("price_limit", 50000) or 50000))
            rec["above_ma60_required"] = bool(ma60 > 0 and live_price > ma60)
            rec["ma60_gap_to_touch"] = (ma60 / live_price - 1) * 100 if live_price and ma60 else rec.get("ma60_gap_to_touch", 999)
            rec["ma60_dist_abs"] = abs(live_price / ma60 - 1) * 100 if live_price and ma60 else rec.get("ma60_dist_abs", 999)
            # 실시간가로 필수조건 재판정: 조건 위반이면 홈/추천 1순위에서 제외합니다.
            base_ok = bool(rec.get("prior_low_hold") and rec.get("near_support") and rec.get("support_ma") != "NONE")
            score_ok = int(rec.get("good_pullback_score", 0) or 0) >= 70
            rec["engine1"] = bool(rec["price_preferred"] and rec["above_ma60_required"] and base_ok and score_ok)
            rec["trust1"] = int(max(0, min(98, rec.get("good_pullback_score", 0)))) if rec.get("engine1") else 0
        else:
            rec["real_time_checked"] = False
        return rec
    except Exception:
        return rec

def _v174_scan_result_summary(payload):
    try:
        return (
            f'검색대상 {payload.get("target_count",0)}개 · 분석완료 {payload.get("analyzed_count",0)}개 · '
            f'제외 {payload.get("excluded_count",0)}개 · 실패 {payload.get("failed_count",0)}개 · '
            f'1호기 {payload.get("future_count",0)}개 · 2C+3B {payload.get("attack_count",0)}개'
        )
    except Exception:
        return "스캔 결과 확인불가"
...
def render_real_scanner_control_v142(data):
    cached = load_real_scanner_v142()
    if cached:
        summary = f'최근 스캔 {cached.get("scanned_at_kst","-")}<br>' + _v174_scan_result_summary(cached)
    else:
    ...
    with c2:
        if cached:
            st.download_button(
                ...
                file_name="real_scanner_v174.json",
...
def _mini_price_chart_svg_v147(points):
    """V169 VERIFIED: 추천종목 미니 봉차트. 20/60/120일선을 실제 polyline으로 모두 표시하고, 우측 라벨을 분리 배치합니다."""
    try:
        import base64
        pts = points or []
        if len(pts) < 10:
        ...
        # 모바일 화면에서도 3개 이평선이 구분되도록 높이와 우측 라벨 영역을 확대합니다.
        w, h = 760, 360
        left, right, top, bottom = 42, 182, 24, 74

        vals = []
        for p in pts:
            for k in ['high', 'low', 'ma20', 'ma60', 'ma120']:
                try:
        ...
        bw = max(3.2, min(7.2, step * 0.50))

        svg = []
        svg.append(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {w} {h}" width="100%" height="360" role="img">')
        svg.append(f'<rect x="0" y="0" width="{w}" height="{h}" rx="16" fill="#ffffff"/>')
        for k in range(5):
            yy = top + k * (h - top - bottom) / 4
            svg.append(f'<line x1="{left}" y1="{yy:.1f}" x2="{w-right}" y2="{yy:.1f}" stroke="#e5e7eb" stroke-width="1"/>')

        candles = []
        ma20_pts, ma60_pts, ma120_pts = [], [], []
        for i, p in enumerate(pts):
            x = left + i * step
            c = float(p.get('close', 0) or 0)
            o = float(p.get('open', c) or c)
            hh = float(p.get('high', c) or c)
            ll = float(p.get('low', c) or c)
            ma20 = float(p.get('ma20', 0) or 0)
            ma60 = float(p.get('ma60', 0) or 0)
            ma120 = float(p.get('ma120', 0) or 0)
            col = '#dc2626' if c >= o else '#2563eb'
            y_hi, y_lo = y(hh), y(ll)
            y_o, y_c = y(o), y(c)
            body_y = min(y_o, y_c)
            body_h = max(2.2, abs(y_c - y_o))
            candles.append(f'<line x1="{x:.1f}" y1="{y_hi:.1f}" x2="{x:.1f}" y2="{y_lo:.1f}" stroke="{col}" stroke-width="1.15" opacity="0.92"/>')
            candles.append(f'<rect x="{x-bw/2:.1f}" y="{body_y:.1f}" width="{bw:.1f}" height="{body_h:.1f}" rx="1.2" fill="{col}" opacity="0.88"/>')
            if ma20 > 0:
        ...
        # 봉을 먼저 그리고, 그 위에 120→60→20 순서로 굵게 올려서 3개 선이 모두 보이게 합니다.
        svg.extend(candles)
        if ma120_pts:
            svg.append(f'<polyline points="{" ".join(ma120_pts)}" fill="none" stroke="#7c3aed" stroke-width="3.6" stroke-linecap="round" stroke-linejoin="round" stroke-dasharray="10 6" opacity="1"/>')
        if ma60_pts:
            svg.append(f'<polyline points="{" ".join(ma60_pts)}" fill="none" stroke="#f97316" stroke-width="3.8" stroke-linecap="round" stroke-linejoin="round" opacity="1"/>')
        if ma20_pts:
            svg.append(f'<polyline points="{" ".join(ma20_pts)}" fill="none" stroke="#16a34a" stroke-width="3.8" stroke-linecap="round" stroke-linejoin="round" opacity="1"/>')
        ...
        labels = []
        if close > 0:
            labels.append({'key': '현재', 'value': close, 'y': y(close), 'fill': '#0f172a', 'text': '#ffffff', 'w': 122})
        for key, value, fill, text in [
            ('20일선', ma20, '#16a34a', '#ffffff'),
            ('60일선', ma60, '#f97316', '#ffffff'),
            ('120일선', ma120, '#7c3aed', '#ffffff'),
        ]:
            if value > 0:
                yy = y(value)
                svg.append(f'<line x1="{left}" y1="{yy:.1f}" x2="{w-right}" y2="{yy:.1f}" stroke="{fill}" stroke-width="1" stroke-dasharray="4 6" opacity="0.22"/>')
                labels.append({'key': key, 'value': value, 'y': yy, 'fill': fill, 'text': text, 'w': 122})

        # 우측 금액 라벨은 실제 y위치와 표시 y위치를 분리해서 겹치지 않게 강제 정렬합니다.
        labels = sorted(labels, key=lambda x: x['y'])
        min_gap = 28
        low_bound, high_bound = top + 14, h - bottom - 18
        placed = []
        for lab in labels:
            ...
            lab['adj_y'] = yy
            placed.append(lab)
        if placed and placed[-1]['adj_y'] > high_bound:
            overflow = placed[-1]['adj_y'] - high_bound
            for lab in placed:
                lab['adj_y'] -= overflow
        ...
        for lab in placed:
            yy = lab['adj_y']
            svg.append(f'<line x1="{w-right}" y1="{lab["y"]:.1f}" x2="{w-right+18}" y2="{yy:.1f}" stroke="{lab["fill"]}" stroke-width="1.15" opacity="0.62"/>')
            svg.append(f'<rect x="{w-right+20}" y="{yy-12:.1f}" width="{lab["w"]}" height="24" rx="12" fill="{lab["fill"]}" opacity="0.98"/>')
            svg.append(f'<text x="{w-right+29}" y="{yy+4.5:.1f}" font-size="11" font-weight="850" fill="{lab["text"]}">{lab["key"]} {price_txt(lab["value"])}</text>')

        if close > 0:
            svg.append(f'<circle cx="{w-right:.1f}" cy="{y(close):.1f}" r="4.4" fill="#0f172a"/>')

        svg.append(f'<text x="{left}" y="{h-34}" font-size="11" font-weight="760" fill="#64748b">최근 {n}봉</text>')
        svg.append(f'<line x1="{left+86}" y1="{h-38}" x2="{left+124}" y2="{h-38}" stroke="#16a34a" stroke-width="4" stroke-linecap="round"/><text x="{left+130}" y="{h-34}" font-size="12" font-weight="900" fill="#16a34a">20일선</text>')
        svg.append(f'<line x1="{left+208}" y1="{h-38}" x2="{left+246}" y2="{h-38}" stroke="#f97316" stroke-width="4" stroke-linecap="round"/><text x="{left+252}" y="{h-34}" font-size="12" font-weight="900" fill="#f97316">60일선</text>')
        svg.append(f'<line x1="{left+330}" y1="{h-38}" x2="{left+368}" y2="{h-38}" stroke="#7c3aed" stroke-width="4" stroke-linecap="round" stroke-dasharray="8 5"/><text x="{left+374}" y="{h-34}" font-size="12" font-weight="900" fill="#7c3aed">120일선</text>')
        svg.append('</svg>')

        svg_text = ''.join(svg)
        b64 = base64.b64encode(svg_text.encode('utf-8')).decode('ascii')
        return (
            '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:16px;padding:10px;margin:10px 0;overflow:hidden;">'
            f'<img src="data:image/svg+xml;base64,{b64}" style="width:100%;height:auto;display:block;border-radius:14px;" />'
            '</div>'
        )
    except Exception:
        return '<div class="brief-sub">차트 표시 오류</div>'
...
def _v176_candidate_score(r):
    """V176: strict 추천 후보가 없을 때도 실제 스캔 결과 중 최우선 관망 후보를 산출합니다.
    생존 필수조건은 따로 표시하고, 매수 추천과 관망 후보를 구분합니다.
    """
    try:
        close = float(r.get('close', 0) or 0)
        ma60 = float(r.get('ma60', 0) or 0)
        price_ok = bool(r.get('price_preferred', close <= 50000))
        ma_ok = bool(r.get('above_ma60_required', ma60 > 0 and close > ma60))
        low_ok = bool(r.get('prior_low_hold'))
        support_ok = bool(r.get('near_support') or r.get('support_ma') != 'NONE')
        score = 0
        reasons = []
        fails = []
        if price_ok:
            score += 18; reasons.append('5만원 이하')
        else:
            fails.append('5만원 초과')
        if ma_ok:
            score += 30; reasons.append('60일선 위')
        else:
            fails.append('60일선 아래')
        if low_ok:
            score += 25; reasons.append('전저점 유지')
        else:
            fails.append('전저점 확인 필요')
        if support_ok:
            score += 15; reasons.append('지지선 근접')
        else:
            fails.append('지지선 부족')
        gp = int(r.get('good_pullback_score', 0) or 0)
        score += int(gp * 0.12)
        score += int(r.get('mtf_score', 0) or 0)
        if r.get('compression_consecutive') or r.get('compression_progress_10'):
            score += 8; reasons.append('압축 진행')
        if float(r.get('rise20', 0) or 0) > 25:
            score -= 15; fails.append('최근 급등')
        score = max(0, min(100, int(score)))
        return {"score": score, "reasons": reasons, "fails": fails, "price_ok": price_ok, "ma_ok": ma_ok, "low_ok": low_ok, "support_ok": support_ok}
    except Exception:
...
def _v175_final_pick_from_scanner(data):
    ...
    1순위: 엄격한 매수 가능 후보
    2순위: strict 후보가 없으면 최우선 관망 후보를 표시해 '왜 추천 없음인지'까지 설명
    """
    try:
        cached = load_real_scanner_v142()
...
def _v175_final_recommend_card(pick, note=""):
    try:
        ...
        pick_type = pick.get('v175_pick_type', '실시간 판단')
...
def render_v173_real_data_status(data):
    ...
    try:
        ...
        st.markdown(f'<div class="brief-card"><div class="brief-title">📡 V174 실시간 데이터</div><div class="brief-sub">상태 확인 실패: {e}</div></div>', unsafe_allow_html=True)



def render_v174_scanner_home_status():
    """홈에서 마지막 시장 스캔 결과를 5초로 확인합니다."""
    try:
        cached = load_real_scanner_v142()
        if not cached or not cached.get("records"):
        ...
        lines = []
        for r in future[:3]:
            lines.append(f'• <b>{r.get("name","-")}</b>: 신뢰도 {r.get("trust1",0)}% · 현재가 {won(r.get("close",0))} · 60일선 {won(r.get("ma60",0))}')
        body = f'최근 스캔 {cached.get("scanned_at_kst","-")}<br>{_v174_scan_result_summary(cached)}'
        if lines:
...
def _v195_operation_command_card(plan):
    ...
    return (
        ...
        f'<div class="v195-command-dark" style="background:#111827!important;color:#ffffff!important;-webkit-text-fill-color:#ffffff!important;border-radius:13px;padding:11px;font-size:14px;font-weight:950;line-height:1.55;margin-bottom:9px;"><span class="v195-command-text" style="color:#ffffff!important;-webkit-text-fill-color:#ffffff!important;background:transparent!important;">현재 행동: {cmd["command"]}</span></div>'
...
# V185 OPERATION MEMORY: 발굴부터 매도/복기까지 기록하는 작전 메모리 강화
# 원칙: 미승인 발굴후보는 절대 보유/추매/비중 계산에 포함하지 않습니다.
def _op_event(op, etype, note='', price=0, amount=0):
    op.setdefault('events', [])
    op['events'].append({
        'time': now_label(),
        'type': etype,
        'note': str(note or ''),
        'price': int(_op_float(price, 0)),
        'amount': int(_op_float(amount, 0)),
    })
...
def _op_profit_snapshot(op, price=None):
    name = norm(op.get('name',''))
    if price is None:
        price, src = operation_current_price(name)
    else:
    ...
    qty = _op_float(op.get('qty'), 0)
    invested = _op_float(op.get('invested'), avg * qty)
    realized = _op_float(op.get('realized'), 0)
    current_value = qty * _op_float(price, 0)
    total_value = realized + current_value
    profit = total_value - invested
    rate = profit / invested * 100 if invested > 0 else 0
    unreal = current_value - (avg * qty) if avg > 0 else 0
    return {
        'name': name, 'price': _op_float(price, 0), 'src': src,
        'avg': avg, 'qty': qty, 'invested': invested, 'realized': realized,
        'current_value': current_value, 'total_value': total_value,
        'profit': profit, 'rate': rate, 'unreal': unreal,
    }
...
def evaluate_staff_operation(op):
    ...
    price, src = operation_current_price(name)
    budget = _op_float(op.get('budget'), 0)
    snap = _op_profit_snapshot(op, price)
    avg, qty, invested, rate = snap['avg'], snap['qty'], snap['invested'], snap['rate']
    stop = _op_float(op.get('stop'), 0)
    target1 = _op_float(op.get('target1'), 0)
    target2 = _op_float(op.get('target2'), 0)
    ...
    if status == 'WATCH':
        return {
            'name': name, 'status': status, 'price': price, 'src': src, 'rate': 0,
            'headline': '👀 발굴후보 추적중',
            'action': '승인 전에는 관찰만 · 보유/추매 계산 제외',
            'detail': f'추천 예산 {won(budget)} · 미진입 상태 · 마음에 들지 않으면 미승인 처리',
            'level': 4,
        }
    if status == 'REJECTED':
        return {'name': name, 'status': status, 'price': price, 'src': src, 'rate': 0, 'headline': '🚫 미승인', 'action': '브리핑/추매/보유 계산 제외', 'detail': '사용자가 승인하지 않은 후보입니다. 작전 진행 없음.', 'level': 9}
    if status == 'DONE':
        final_rate = _op_float(op.get('final_rate'), snap['rate'])
        final_profit = _op_float(op.get('final_profit'), snap['profit'])
        return {'name': name, 'status': status, 'price': price, 'src': src, 'rate': final_rate, 'headline': '🏁 작전종료', 'action': '복기 대상', 'detail': f'최종수익 {won(final_profit)} · 최종수익률 {final_rate:+.2f}%<br>{op.get("memo", "종료된 작전입니다.")}', 'level': 8}

    # ACTIVE/EXITING: 평단 기반 작전 책임 모드
    if stop and price and price <= stop:
        headline, action, level = '🔴 손절선 도달', '전량 또는 절반 매도 검토', 0
    elif rate <= -8:
        headline, action, level = '🔴 손실 확대', '추매 금지 · 손절/비중축소 검토', 0
    ...
    if status == 'EXITING':
        headline = '🟠 수확/철수 검토중'
        action = '추가매수 금지 · 매도 계획 우선'
        level = min(level, 1)

    remain_budget = max(0, budget - invested)
    detail = f'평단 {won(avg)} · 현재가 {won(price)} · 작전수익률 {rate:+.2f}%<br>투입 {won(invested)} / 작전예산 {won(budget)} · 남은 작전여력 {won(remain_budget)}'
    if snap['realized']:
        detail += f'<br>실현금액 {won(snap["realized"])} · 현재평가 {won(snap["current_value"])} · 총손익 {won(snap["profit"])}'
    if stop:
        detail += f'<br>손절선 {won(stop)}'
    if target1:
        detail += f' · 1차목표 {won(target1)}'
...
def render_staff_operations_home(data=None):
    ...
    if not ops:
        st.markdown(
            '<div class="brief-card"><div class="brief-title">🏛 참모 작전 메모리</div>'
            '<div class="brief-sub">아직 등록된 작전이 없습니다. 발굴 후보를 승인해야 평단·추매·매도 추적이 시작됩니다.</div></div>',
            unsafe_allow_html=True
        )
        return
    evals = [evaluate_staff_operation(x) for x in active[:6]]
    urgent = [x for x in evals if x.get('level', 9) <= 1]
    if urgent:
    ...
    for e in evals[:4]:
        lines.append(f'• <b>{e["name"]}</b>: {e["headline"]}<br>&nbsp;&nbsp;→ {e["action"]}<br>&nbsp;&nbsp;{e["detail"]}')
    if watch:
        lines.append(f'• 미승인 발굴후보: {", ".join([norm(x.get("name","")) for x in watch[:4]])}<br>&nbsp;&nbsp;→ 보유/추매 계산 제외')
    if done:
...
def _render_op_event_history(op):
    events = op.get('events', []) or []
    if not events:
        st.caption('아직 행동 로그가 없습니다.')
        return
    st.markdown('<div class="brief-card"><div class="brief-title">📜 행동 로그</div>', unsafe_allow_html=True)
    for ev in reversed(events[-12:]):
        typ = ev.get('type','')
        p = _op_float(ev.get('price'),0)
        a = _op_float(ev.get('amount'),0)
        note = ev.get('note','')
        line = f'{ev.get("time","")} · {typ}'
        if p or a:
            line += f' · {won(p)} · {won(a)}'
        if note:
            line += f'<br>{note}'
        st.markdown(f'<div class="brief-sub">{line}</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
def render_staff_operations_panel(data=None):
    ...
    if can_write_operations():
        with st.expander('➕ 발굴 후보 / 작전 직접 등록', expanded=False):
            c1, c2 = st.columns([2,1])
            with c1:
                name = st.text_input('종목명', value='', placeholder='예: 서울반도체', key='op_new_name_v185')
            with c2:
                budget = st.number_input('작전예산', min_value=0, max_value=100000000, value=1000000, step=100000, format='%d', key='op_new_budget_v185')
            memo = st.text_input('발굴 메모', value='', placeholder='예: 120일선 지지 + 거래량 증가', key='op_new_memo_v185')
            if st.button('👀 발굴 후보로 등록', use_container_width=True, key='op_add_watch_v185'):
                if name.strip():
                    new_op = {
                        'id': _next_operation_id(ops), 'name': norm(name), 'status': 'WATCH', 'budget': int(budget),
                        'memo': memo, 'created_at': now_label(), 'approved_at': '', 'closed_at': '', 'events': []
                    }
    ...
    status_rank = {'ACTIVE':0, 'EXITING':0, 'WATCH':1, 'REJECTED':2, 'DONE':3}
    ops_sorted = sorted(ops, key=lambda x: (status_rank.get(x.get('status','WATCH'), 9), x.get('created_at','')), reverse=False)
    changed = False
    ...
    with tab_active:
        active_ops = [x for x in ops_sorted if x.get('status') in ['ACTIVE','EXITING']]
        if not active_ops:
        ...
        for op in active_ops:
            ...
            if can_write_operations():
                with st.expander(f'추매/매도/종료 기록 · {e["name"]}', expanded=False):
                    c1, c2 = st.columns(2)
                    with c1:
                        add_price = st.number_input('추가 매수가', min_value=0, max_value=10000000, value=int(e.get('price') or 0), step=10, format='%d', key=f'op_add_price_v185_{oid}')
                        add_amount = st.number_input('추가 투입금', min_value=0, max_value=100000000, value=0, step=10000, format='%d', key=f'op_add_amt_v185_{oid}')
                    with c2:
                        sell_price = st.number_input('매도가', min_value=0, max_value=10000000, value=int(e.get('price') or 0), step=10, format='%d', key=f'op_sell_price_v185_{oid}')
                        sell_amount = st.number_input('매도금액', min_value=0, max_value=100000000, value=0, step=10000, format='%d', key=f'op_sell_amt_v185_{oid}')
                    b1, b2, b3, b4 = st.columns(4)
                    if b1.button('➕ 추매', use_container_width=True, key=f'op_add_buy_v185_{oid}') and add_price and add_amount:
                        old_inv = _op_float(op.get('invested'), 0); old_qty = _op_float(op.get('qty'), 0)
                        new_qty = float(add_amount) / float(add_price)
                        op['invested'] = int(old_inv + add_amount); op['qty'] = round(old_qty + new_qty, 4)
                        op['avg'] = int(op['invested'] / op['qty']) if op['qty'] else op.get('avg', 0)
                        _op_event(op, 'BUY', '추매 반영', add_price, add_amount); changed = True
                    if b2.button('➖ 부분매도', use_container_width=True, key=f'op_sell_v185_{oid}') and sell_price and sell_amount:
                        old_qty = _op_float(op.get('qty'), 0)
                        sell_qty = float(sell_amount) / float(sell_price) if sell_price else 0
                        op['qty'] = round(max(0, old_qty - sell_qty), 4)
                        op['realized'] = _op_float(op.get('realized'), 0) + float(sell_amount)
                        op['status'] = 'EXITING' if op['qty'] > 0 else 'DONE'
                        if op['status'] == 'DONE':
                            snap = _op_profit_snapshot(op, sell_price)
                            op['final_profit'] = snap['profit']; op['final_rate'] = snap['rate']; op['closed_at'] = now_label()
                        _op_event(op, 'SELL', '부분매도 반영', sell_price, sell_amount); changed = True
                    if b3.button('🟠 철수검토', use_container_width=True, key=f'op_exiting_v185_{oid}'):
                        op['status'] = 'EXITING'; _op_event(op, 'EXIT_PLAN', '수확/철수 검토 상태 전환'); changed = True
                    if b4.button('🏁 종료', use_container_width=True, key=f'op_done_v185_{oid}'):
                        snap = _op_profit_snapshot(op, e.get('price'))
                        op['status'] = 'DONE'; op['closed_at'] = now_label(); op['final_profit'] = snap['profit']; op['final_rate'] = snap['rate']
                        _op_event(op, 'DONE', f'작전 종료 · 최종수익률 {snap["rate"]:+.2f}%'); changed = True

    with tab_watch:
        watch_ops = [x for x in ops_sorted if x.get('status','WATCH') in ['WATCH','REJECTED']]
        if not watch_ops:
            st.info('발굴/승인대기 후보가 없습니다.')
        for op in watch_ops:
            e = evaluate_staff_operation(op); oid = op.get('id')
            st.markdown(op_card(op,e), unsafe_allow_html=True)
            _render_op_event_history(op)
            if can_write_operations() and op.get('status','WATCH') == 'WATCH':
                with st.expander(f'작전 승인 / 제외 · {e["name"]}', expanded=False):
                    c1, c2 = st.columns(2)
                    with c1:
                        avg = st.number_input('1차 매수가/평단', min_value=0, max_value=10000000, value=int(e.get('price') or 0), step=10, format='%d', key=f'op_avg_v185_{oid}')
                        amount = st.number_input('1차 투입금', min_value=0, max_value=100000000, value=min(int(_op_float(op.get('budget'),1000000)*0.5), int(_op_float(op.get('budget'),1000000))), step=10000, format='%d', key=f'op_amt_v185_{oid}')
                    with c2:
                        stop = st.number_input('손절선', min_value=0, max_value=10000000, value=int(avg*0.92) if avg else 0, step=10, format='%d', key=f'op_stop_v185_{oid}')
                        target1 = st.number_input('1차 목표가', min_value=0, max_value=10000000, value=int(avg*1.15) if avg else 0, step=10, format='%d', key=f'op_t1_v185_{oid}')
                        target2 = st.number_input('2차 목표가', min_value=0, max_value=10000000, value=int(avg*1.30) if avg else 0, step=10, format='%d', key=f'op_t2_v185_{oid}')
                    b1, b2 = st.columns(2)
                    if b1.button('🟢 작전 승인', use_container_width=True, key=f'op_approve_v185_{oid}'):
                        op['status'] = 'ACTIVE'; op['avg'] = int(avg); op['invested'] = int(amount)
                        op['qty'] = round((float(amount) / float(avg)), 4) if avg else 0
                        op['stop'] = int(stop); op['target1'] = int(target1); op['target2'] = int(target2)
                        op['approved_at'] = now_label(); _op_event(op, 'APPROVE', '사용자 승인 · 작전 시작', avg, amount); changed = True
                    if b2.button('🚫 미승인/제외', use_container_width=True, key=f'op_reject_v185_{oid}'):
                        op['status'] = 'REJECTED'; op['closed_at'] = now_label(); _op_event(op, 'REJECT', '사용자 미승인 · 추적 제외'); changed = True

    with tab_done:
        done_ops = [x for x in ops_sorted if x.get('status') == 'DONE']
        if not done_ops:
            st.info('종료된 작전이 없습니다.')
        for op in done_ops:
            e = evaluate_staff_operation(op)
            st.markdown(op_card(op,e), unsafe_allow_html=True)
            _render_op_event_history(op)
