...
APP_TITLE = "🧭 스톡 컴퍼스 V78-5"
APP_SUBTITLE = "경규님 전용 개인용 AI 투자비서 · 매수타이밍 엔진 1차"
...
st.set_page_config(page_title="스톡 컴퍼스 V78-5", page_icon="🧭", layout="centered")
...
def css():
    ...
    .buytiming-sub{font-size:12px;font-weight:850;color:#64748b!important;-webkit-text-fill-color:#64748b!important;margin-top:4px;line-height:1.4}
    .buytiming-score{text-align:right;min-width:76px}
    .buytiming-score .num{font-size:31px;font-weight:950;color:#020617!important;-webkit-text-fill-color:#020617!important;line-height:1}
    .buytiming-score .txt{font-size:12px;font-weight:900;color:#64748b!important;-webkit-text-fill-color:#64748b!important;margin-top:4px}
    ...
    .buytiming-action *{color:#fff!important;-webkit-text-fill-color:#fff!important}
    .buytiming-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:10px 0}
    .buytiming-box{background:#f1f5f9;border:1px solid #e2e8f0;border-radius:14px;padding:10px}
    .buytiming-label{font-size:11px;font-weight:850;color:#64748b!important;-webkit-text-fill-color:#64748b!important;margin-bottom:4px}
    .buytiming-value{font-size:15px;font-weight:950;color:#020617!important;-webkit-text-fill-color:#020617!important}
    .buytiming-bar{width:100%;height:12px;background:#e2e8f0;border-radius:999px;overflow:hidden;margin:8px 0 4px}
    .buytiming-fill{height:100%;border-radius:999px;background:linear-gradient(90deg,#ef4444 0%,#f59e0b 45%,#22c55e 100%)}
    ...
    .buytiming-list{background:#fff;border:1px solid #e2e8f0;border-radius:16px;padding:11px 12px;margin:8px 0}
    .buytiming-list-head{display:flex;justify-content:space-between;gap:8px}
    .buytiming-list-name{font-size:15px;font-weight:950;color:#020617!important;-webkit-text-fill-color:#020617!important}
    .buytiming-list-score{font-size:18px;font-weight:950;color:#020617!important;-webkit-text-fill-color:#020617!important;white-space:nowrap}
    .buytiming-list-meta{font-size:12px;font-weight:850;color:#64748b!important;-webkit-text-fill-color:#64748b!important;margin-top:5px;line-height:1.45}
    @media(max-width:700px){.buytiming-card{padding:15px!important;border-radius:21px!important}.buytiming-title{font-size:19px}.buytiming-grid{grid-template-columns:1fr 1fr}.buytiming-score .num{font-size:28px}}
...
def buy_timing_level(score):
    try:
        s = int(score)
    except Exception:
        s = 50
    if s >= 85:
        return "🟢 적극매수권", "오늘 1주 또는 소액 분할매수 가능"
    if s >= 70:
        return "🟡 분할매수권", "한 번에 몰지 말고 분할매수"
    if s >= 55:
        return "🟡 보유권", "신규매수보다 보유 유지"
    if s >= 40:
        return "🟠 관망권", "추가매수 보류 후 추세 확인"
    return "🔴 매수금지권", "비중 확대 금지 · 리스크 점검"

def buy_timing_analysis(name, result=None, data=None):
    n = norm(name)
    if result is None:
        try:
            md = fetch_market_data(n)
            p = md.get("price")
            result = {"price": p, "change_rate": md.get("change_rate"), "volume": md.get("volume"),
                      "pos52": position_52w(p, md.get("low_52w"), md.get("high_52w"))}
        except Exception:
            result = {}
    ...
    try:
        e = lifecycle_engine(n, price, result)
    except Exception:
        e = {"stage":"🟡 성장", "entry":"🟠 관망", "rumor_risk":50, "upside_prob":55, "reasons":["데이터 제한으로 보수적으로 판단했습니다."]}

    upside = int(e.get("upside_prob", 55) or 55)
    rumor = int(e.get("rumor_risk", 50) or 50)
    pos52 = e.get("pos52")
    chg20 = e.get("chg20")
    reasons = list(e.get("reasons", []))[:3]

    # 매수적합도 = 상승확률 + 낮은 선반영위험 + 위치/가격부담 보정
    score = 50
    score += int((upside - 50) * 0.55)
    score -= int(max(0, rumor - 45) * 0.35)

    if pos52 is not None:
        if pos52 <= 35:
            score += 10
            reasons.append(f"가격 위치 {pos52:.0f}%로 상단 추격 부담이 낮습니다.")
        elif pos52 >= 80:
            score -= 12
            reasons.append(f"가격 위치 {pos52:.0f}%로 추격매수 부담이 있습니다.")

    if chg20 is not None:
        if chg20 >= 20:
            score -= 12
            reasons.append(f"최근 20일 {chg20:.1f}% 상승으로 단기 과열을 반영했습니다.")
        elif -8 <= chg20 <= 8:
            score += 6
            reasons.append("최근 등락이 과하지 않아 분할 접근이 가능합니다.")

    try:
        cr = result.get("change_rate")
        if cr is not None:
            if cr >= 7:
                score -= 8
                reasons.append(f"당일 {cr:.2f}% 상승으로 당일 추격은 신중합니다.")
            elif cr <= -5:
                score -= 4
                reasons.append(f"당일 {cr:.2f}% 하락으로 추가 하락 확인이 필요합니다.")
    except Exception:
        pass
    ...
    try:
        if data:
            ...
            if sw >= 55:
                score -= 10
                reasons.append(f"{sec} 비중 {sw:.1f}%로 포트 집중도 감점입니다.")
            elif sw <= 20:
                score += 4
                reasons.append(f"{sec} 비중이 낮아 분산 보강 여지가 있습니다.")
    except Exception:
        pass

    score = max(0, min(100, int(score)))
    level, action = buy_timing_level(score)

    risk_txt = "낮음" if rumor <= 35 else ("보통" if rumor <= 65 else "높음")
    return {
        "name": n,
        "score": score,
        "level": level,
        ...
        "pos52": pos52,
        "chg20": chg20,
        "reasons": reasons[:5] or ["현재 데이터 기준으로 보수적으로 판단했습니다."],
    }

def render_buy_timing_card(name, result=None, data=None, title_prefix="⏱️ 매수타이밍"):
    t = buy_timing_analysis(name, result, data)
    reason_html = "<br>".join([f"① {x}" for x in t.get("reasons", [])])
    pos_txt = "-" if t.get("pos52") is None else f'{t["pos52"]:.0f}%'
    chg_txt = "-" if t.get("chg20") is None else f'{t["chg20"]:.1f}%'

    html = (
        '<div class="buytiming-card">'
        '<div class="buytiming-top">'
        f'<div><div class="buytiming-title">{title_prefix} · {t["name"]}</div>'
        f'<div class="buytiming-sub">{t["level"]}</div></div>'
        f'<div class="buytiming-score"><div class="num">{t["score"]}</div><div class="txt">매수적합도</div></div>'
        '</div>'
        f'<div class="buytiming-action">오늘 행동: {t["action"]}</div>'
        f'<div class="buytiming-bar"><div class="buytiming-fill" style="width:{t["score"]}%"></div></div>'
        '<div class="buytiming-grid">'
        f'<div class="buytiming-box"><div class="buytiming-label">현재위치</div><div class="buytiming-value">{t["stage"]}</div></div>'
        f'<div class="buytiming-box"><div class="buytiming-label">진입타이밍</div><div class="buytiming-value">{t["entry"]}</div></div>'
        f'<div class="buytiming-box"><div class="buytiming-label">상승확률</div><div class="buytiming-value">{t["upside"]}%</div></div>'
        f'<div class="buytiming-box"><div class="buytiming-label">선반영위험</div><div class="buytiming-value">{t["risk_txt"]} · {t["rumor"]}%</div></div>'
        '</div>'
        f'<div class="buytiming-reason"><b>판단근거</b><br>{reason_html}<br>52주/최근 위치: {pos_txt} · 최근 20일 등락: {chg_txt}</div>'
        '</div>'
    )
    st.markdown(html, unsafe_allow_html=True)
...
def holding_buy_timing_list(data):
    ...
    for n, q, a, r in rows:
        try:
            t = buy_timing_analysis(n, r, data)
            out.append(t)
        except Exception:
            pass
...
def render_buy_timing_summary(data):
    ...
    if not items:
        ...
        return
    top = items[0]
    render_buy_timing_card(top["name"], None, data, title_prefix="⏱️ 보유종목 매수타이밍 1순위")
...
def render_buy_timing_ranking(data):
    ...
    for idx, x in enumerate(items, start=1):
        ...
        row = (
            ...
            f'<div class="buytiming-list-name">{medal} {x["name"]}</div>'
            f'<div class="buytiming-list-score">{x["score"]}점</div>'
            '</div>'
            f'<div class="buytiming-list-meta">{x["level"]} · {x["action"]}<br>현재위치 {x["stage"]} · 상승확률 {x["upside"]}% · 선반영위험 {x["risk_txt"]}</div>'
            '</div>'
        )
