...
def _vg_append_ledger(result):
    try:
        old=_vg_read(VALIDATOR_LEDGER_FILE)
        rows=old.get("rows",[]) if isinstance(old,dict) else []
        d=result.get("decision",{}) or {}
        rows.append({
            "created_at":result.get("created_at",""),
            "base_lock":result.get("base_lock",""),
            "experiment":result.get("experiment",""),
            "status":d.get("status",""),
            "feature":d.get("label",""),
            "threshold":"",
            "blind_delta":d.get("blind_delta",0),
            "retain":100.0,
            "note":d.get("reason",""),
        })
        _vg_write(VALIDATOR_LEDGER_FILE,{"rows":rows[-50:]})
    except:pass

def _vg_summary(rows):
    n=len(rows);vc=Counter(str(x.get("outcome","")) for x in rows)
    if not n:
        return {"n":0,"win_rate":0.0,"stop_rate":0.0,"timeout_rate":0.0,
                "avg_return":0.0,"avg_days":0.0}
    return {
...
def _vg_monthly_rate(rows,start_date,end_date):
    try:
        a=pd.Timestamp(start_date);b=pd.Timestamp(end_date)
        months=max(1,(b.year-a.year)*12+(b.month-a.month)+1)
...
def _vg_add_breadth(agg,df,market,test_start,test_end):
    """
    시장지수 대신 같은 적격 종목군의 실제 시장폭을 사용한다.
    신호일 종가까지만 계산하므로 미래정보 없음.
    1) 종가 > 20MA 비율
    2) 20MA 상승 비율
    3) 최근10일 저점 >= 직전10일 저점 비율
    """
    try:
        q=df.copy().reset_index(drop=True)
        if len(q)<40:return
        q["date"]=pd.to_datetime(q["date"]).dt.normalize()
        c=q.close.astype(float);lo=q.low.astype(float)
        ma20=c.rolling(20).mean()
        low10=lo.rolling(10).min()
        prev10=lo.shift(10).rolling(10).min()
        c1=c>=ma20
        c2=ma20>ma20.shift(5)
        c3=low10>=prev10
        valid=ma20.notna() & ma20.shift(5).notna() & prev10.notna()
        mask=valid & (q["date"]>=pd.Timestamp(test_start)) & (q["date"]<=pd.Timestamp(test_end))
        for j in q.index[mask]:
            d=str(pd.Timestamp(q.loc[j,"date"]).date())
            key=f"{market}|{d}"
            r=agg.setdefault(key,{"n":0,"c1":0,"c2":0,"c3":0})
            r["n"]+=1
            r["c1"]+=int(bool(c1.loc[j]));r["c2"]+=int(bool(c2.loc[j]));r["c3"]+=int(bool(c3.loc[j]))
    except:pass

def _vg_finalize_breadth(agg):
    out={}
    for key,r in agg.items():
        n=int(r.get("n",0))
        if n<40:continue
        p1=r.get("c1",0)/n*100.0;p2=r.get("c2",0)/n*100.0;p3=r.get("c3",0)/n*100.0
        score=int(p1>=50.0)+int(p2>=50.0)+int(p3>=50.0)
        out[key]={"score":score,"n":n,"above20":round(p1,1),"ma20_up":round(p2,1),"higher_low":round(p3,1)}
    return out

def _vg_split(rows):
    dates=sorted(set(x["signal_date"] for x in rows))
    if len(dates)<8:return None,None,[],[],[]
...
def _vg_filter(rows,threshold):
    return [x for x in rows if int(x.get("market_score",-1))>=int(threshold)]

def _vg_stability(rows,threshold):
    """같은 기준으로 시간순 4구간 중 BASE 이상인 구간 수."""
    dates=sorted(set(x["signal_date"] for x in rows))
    if len(dates)<8:return {"passes":0,"folds":[]}
    folds=[];passes=0
    for k in range(4):
        a=int(len(dates)*k/4);b=int(len(dates)*(k+1)/4)
        dd=set(dates[a:b])
        base=[x for x in rows if x["signal_date"] in dd]
        filt=_vg_filter(base,threshold)
        bs=_vg_summary(base);fs=_vg_summary(filt)
        delta=round(fs["win_rate"]-bs["win_rate"],1)
        ok=bool(fs["n"]>=5 and delta>=0)
        passes+=int(ok)
        folds.append({"fold":k+1,"base_n":bs["n"],"base_win":bs["win_rate"],
                      "filter_n":fs["n"],"filter_win":fs["win_rate"],"delta":delta,"pass":ok})
    return {"passes":passes,"folds":folds}

def _vg_select_market_threshold(train_rows):
    base=_vg_summary(train_rows)
    cand=[]
    for t in (1,2,3):
        rr=_vg_filter(train_rows,t)
        retain=len(rr)/len(train_rows)*100.0 if train_rows else 0.0
        if len(rr)<12 or retain<60.0:continue
        sm=_vg_summary(rr)
        objective=(sm["win_rate"]-base["win_rate"]) + 0.45*(base["stop_rate"]-sm["stop_rate"]) + 0.30*(sm["avg_return"]-base["avg_return"])
        cand.append({"threshold":t,"summary":sm,"retain":round(retain,1),"objective":round(objective,3)})
    if not cand:return {"ok":False,"error":"TRAIN에서 표본 60% 이상을 유지하는 유효 기준 없음"}
    cand.sort(key=lambda x:(x["objective"],x["summary"]["win_rate"],-x["summary"]["stop_rate"],x["retain"]),reverse=True)
    return {"ok":True,"best":cand[0],"candidates":cand}

def _vg_decision(all_rows,train_rows,valid_rows,blind_rows,threshold,start_date,end_date):
    base_va=_vg_summary(valid_rows);fil_va=_vg_summary(_vg_filter(valid_rows,threshold))
    base_bl=_vg_summary(blind_rows);fil_bl=_vg_summary(_vg_filter(blind_rows,threshold))
    all_filtered=_vg_filter(all_rows,threshold)
    retain=len(all_filtered)/len(all_rows)*100.0 if all_rows else 0.0
    monthly=_vg_monthly_rate(all_filtered,start_date,end_date)
    valid_delta=round(fil_va["win_rate"]-base_va["win_rate"],1)
    blind_delta=round(fil_bl["win_rate"]-base_bl["win_rate"],1)
    stable=_vg_stability(all_rows,threshold)

    checks={
        "VALID 악화 없음": bool(fil_va["n"]>=8 and valid_delta>=0.0),
        "BLIND 표본 10건 이상": bool(fil_bl["n"]>=10),
        "BLIND +10% 50% 이상": bool(fil_bl["win_rate"]>=50.0),
        "BLIND BASE 대비 +5%p": bool(blind_delta>=5.0),
        "BLIND A손절 15% 이하": bool(fil_bl["stop_rate"]<=15.0),
        "BLIND A손절 BASE 이하": bool(fil_bl["stop_rate"]<=base_bl["stop_rate"]),
        "신호 60% 이상 유지": bool(retain>=60.0),
        "월평균 4~8건": bool(4.0<=monthly<=8.0),
        "4구간 중 3구간 이상 비악화": bool(stable["passes"]>=3),
    }
    passed=all(checks.values())
    reason="통과 · BASE 승격 검토 가능" if passed else "폐기/보류 · 생산 BASE 변경 금지"
    return {
        "status":"PASS" if passed else "REJECT",
        "reason":reason,
        "checks":checks,
        "base_valid":base_va,"filtered_valid":fil_va,"valid_delta":valid_delta,
        "base_blind":base_bl,"filtered_blind":fil_bl,"blind_delta":blind_delta,
        "retention":round(retain,1),"monthly_rate":monthly,"stability":stable,
    }
...
FAILURE_FEATURES={
    "risk_to_a":{"label":"A까지 위험거리","unit":"%"},
    "b_fresh_days":{"label":"B 신선도","unit":"일"},
    "b_above_a_pct":{"label":"A→B 높이","unit":"%"},
    "b_to_signal_pct":{"label":"B→신호 상승폭","unit":"%"},
    "value_accel":{"label":"거래대금 변화","unit":"배"},
}

def _fc_signal_features(hist,sig):
    """
    모두 신호 당일 종가까지 존재한 정보만 사용한다.
    실제 결과(WIN/STOP/TIMEOUT)는 이후 시뮬레이션에서만 붙인다.
    """
    try:
        ...
        bi=int(sig["B"]["i"])
        cur=float(h.iloc[-1].close)
        if A<=0 or B<=0 or cur<=0 or bi<0 or bi>=n:return None

        # 1) 신호 종가 기준 A까지 위험거리
        risk_to_a=(cur/A-1.0)*100.0

        # 2) B 확인 이후 신호까지 걸린 거래일
        b_fresh_days=int((n-1)-bi)

        # 3) B가 A에서 얼마나 떠 있는지
        b_above_a=(B/A-1.0)*100.0

        # 4) B 저점에서 신호 종가까지 실제 상승폭
        b_to_signal=(cur/B-1.0)*100.0

        # 5) 신호 직전 거래대금이 평소 대비 얼마나 붙었는지
        ...
        return {
            "risk_to_a":round(float(risk_to_a),5),
            "b_fresh_days":int(b_fresh_days),
            "b_above_a_pct":round(float(b_above_a),5),
            "b_to_signal_pct":round(float(b_to_signal),5),
            "value_accel":round(float(value_accel),5),
        }
    except:
        return None

def _fc_median(rows,key):
    vals=[]
    for x in rows:
        try:
            v=float(x.get(key))
...
def _fc_auc(rows,key):
    """
    TRAIN에서 feature 하나가 WIN과 실패를 얼마나 분리하는지 rank-AUC로 측정.
    0.5=무분리, 1.0=완전분리.
    방향은 자동 결정한다.
    """
    arr=[]
    for x in rows:
        try:
            v=float(x.get(key))
            if np.isfinite(v):
                arr.append((v,1 if x.get("outcome")=="WIN" else 0))
        except:pass
    if len(arr)<12:return None
    pos=sum(y for _,y in arr); neg=len(arr)-pos
    if pos<4 or neg<4:return None
    ...
    return {
        "raw_auc":round(float(auc),4),
        "auc":round(float(strength),4),
        "direction":"높을수록 WIN" if higher else "낮을수록 WIN",
        "higher_better":bool(higher),
        "separation":round((float(strength)-0.5)*2.0*100.0,1),
    }

def _fc_feature_table(train_rows):
    wins=[x for x in train_rows if x.get("outcome")=="WIN"]
    stops=[x for x in train_rows if x.get("outcome")=="STOP"]
    timeouts=[x for x in train_rows if x.get("outcome")=="TIMEOUT"]
    fails=[x for x in train_rows if x.get("outcome")!="WIN"]
    out=[]
    for key,meta in FAILURE_FEATURES.items():
        auc=_fc_auc(train_rows,key)
        if not auc:continue
        out.append({
            "key":key,
            "label":meta["label"],
            "unit":meta["unit"],
            "win_median":_fc_median(wins,key),
            "fail_median":_fc_median(fails,key),
            "stop_median":_fc_median(stops,key),
            "timeout_median":_fc_median(timeouts,key),
            **auc,
        })
    out.sort(key=lambda x:(x["auc"],x["separation"]),reverse=True)
    return out

def _fc_apply(rows,key,threshold,higher_better):
    out=[]
    for x in rows:
        try:
            v=float(x.get(key))
            ok=(v>=float(threshold)) if higher_better else (v<=float(threshold))
            if ok:out.append(x)
        except:pass
    return out

def _fc_select_threshold(train_rows,key,higher_better):
    """
    strongest feature 하나에서만 TRAIN threshold를 찾는다.
    과최적화 방지를 위해 TRAIN 신호 60% 이상 유지.
    """
    vals=sorted(float(x[key]) for x in train_rows if key in x and np.isfinite(float(x[key])))
    if len(vals)<15:return {"ok":False,"error":"TRAIN 표본 부족"}

    base=_vg_summary(train_rows)
    min_n=max(12,int(len(train_rows)*0.60))
    qset=[0.20,0.25,0.30,0.35,0.40]
    cand=[]

    for q in qset:
        # higher-better면 아래쪽 q를 잘라내고, lower-better면 위쪽 q를 잘라낸다.
        qq=q if higher_better else 1.0-q
        j=min(len(vals)-1,max(0,int(round((len(vals)-1)*qq))))
        t=vals[j]
        rr=_fc_apply(train_rows,key,t,higher_better)
        if len(rr)<min_n:continue
        sm=_vg_summary(rr)
        retain=len(rr)/len(train_rows)*100.0
        objective=(
            (sm["win_rate"]-base["win_rate"])*1.0 +
            (base["stop_rate"]-sm["stop_rate"])*0.45 +
            (sm["avg_return"]-base["avg_return"])*0.35 -
            max(0.0,70.0-retain)*0.05
        )
        cand.append({
            "threshold":round(float(t),4),
            "summary":sm,
            "retain":round(retain,1),
            "objective":round(float(objective),3),
        })
    if not cand:return {"ok":False,"error":"유효 threshold 없음"}
    cand.sort(key=lambda x:(x["objective"],x["summary"]["win_rate"],-x["summary"]["stop_rate"],x["retain"]),reverse=True)
    return {"ok":True,"best":cand[0],"candidates":cand}

def _fc_validate(all_rows,train_rows,valid_rows,blind_rows,feature_row,threshold_info,start_date,end_date):
    key=feature_row["key"]; higher=feature_row["higher_better"]
    t=float(threshold_info["best"]["threshold"])

    base_tr=_vg_summary(train_rows)
    base_va=_vg_summary(valid_rows)
    base_bl=_vg_summary(blind_rows)

    trf=_fc_apply(train_rows,key,t,higher)
    vaf=_fc_apply(valid_rows,key,t,higher)
    blf=_fc_apply(blind_rows,key,t,higher)
    allf=_fc_apply(all_rows,key,t,higher)

    ftr=_vg_summary(trf); fva=_vg_summary(vaf); fbl=_vg_summary(blf)
    valid_delta=round(fva["win_rate"]-base_va["win_rate"],1)
    blind_delta=round(fbl["win_rate"]-base_bl["win_rate"],1)
    retain_all=round(len(allf)/len(all_rows)*100.0,1) if all_rows else 0.0

    # FAST 연구단계 PASS:
    # VALID/BLIND 둘 다 방향 일치, BLIND +5%p, 손절 악화 없음, 신호 60% 유지.
    checks={
        "VALID 개선 ≥0%p": valid_delta>=0.0,
        "BLIND 개선 ≥5%p": blind_delta>=5.0,
        "BLIND 손절 악화 없음": fbl["stop_rate"]<=base_bl["stop_rate"]+0.1,
        "전체 신호 60% 이상 유지": retain_all>=60.0,
        "BLIND 표본 8건 이상": fbl["n"]>=8,
    }
    ...
    return {
        ...
        "threshold":t,
        "base_train":base_tr,"filtered_train":ftr,
        "base_valid":base_va,"filtered_valid":fva,
        "base_blind":base_bl,"filtered_blind":fbl,
        "valid_delta":valid_delta,"blind_delta":blind_delta,
        "retain_all":retain_all,
        "monthly_rate":_vg_monthly_rate(allf,start_date,end_date),
        "checks":checks,
        "reason":(
            "패배원인 후보가 VALID/BLIND에서도 재현됨 · 전종목 최종검증 대상"
            if status=="PASS"
            else "TRAIN에서 발견된 패턴이 VALID/BLIND에서 충분히 재현되지 않음 · 폐기"
        ),
    }
...
TOPRANK_FEATURES={
    "risk_to_a":{"label":"A위험거리"},
    "b_fresh_days":{"label":"B신선도"},
    "b_above_a_pct":{"label":"A→B높이"},
    "b_to_signal_pct":{"label":"B→신호상승폭"},
    "value_accel":{"label":"거래대금변화"},
}

def _tr_percentile(vals,v):
    try:
        arr=sorted(float(x) for x in vals if np.isfinite(float(x)))
        if not arr:return 50.0
        import bisect
...
def _tr_train_feature_models(train_rows):
    """
    TRAIN에서 각 특징의 WIN 방향과 분리력을 학습한다.
    BLIND/VALID 결과는 절대 사용하지 않는다.
    weight = rank-AUC 분리력, 방향도 TRAIN에서만 결정.
    """
    models={}
    for key,meta in TOPRANK_FEATURES.items():
        auc=_fc_auc(train_rows,key)
        if not auc:
            continue
        weight=max(0.0,float(auc["auc"])-0.5)
        models[key]={
            "label":meta["label"],
            "higher_better":bool(auc["higher_better"]),
            "weight":round(weight,6),
            "auc":float(auc["auc"]),
            "direction":auc["direction"],
            "train_vals":[float(x[key]) for x in train_rows if key in x and np.isfinite(float(x[key]))],
        }
    return models

def _tr_score_row(row,models):
    """
    같은 날 종목끼리 비교할 때 사용할 점수.
    각 특징을 TRAIN 분포 percentile로 환산한 뒤
    TRAIN에서 실제 분리력이 컸던 특징에만 더 큰 가중치.
    """
    num=0.0; den=0.0; detail={}
    for key,m in models.items():
        p=_tr_percentile(m["train_vals"],row.get(key,0))
        good=p if m["higher_better"] else 100.0-p
        w=max(0.001,float(m["weight"]))
        num += good*w
        den += w
        detail[key]=round(good,1)
    score=(num/den) if den>0 else 50.0
    return round(float(score),2),detail

def _tr_group_by_day(rows):
    groups={}
    for x in rows:
        groups.setdefault(x["signal_date"],[]).append(x)
    return groups

def _tr_rank_rows(rows,models):
    """
    모든 BASE 신호는 유지한다.
    단지 같은 날짜 안에서 1위/2위/3위를 매긴다.
    """
    groups=_tr_group_by_day(rows)
    ranked=[]
    for d,arr in groups.items():
        scored=[]
        for x in arr:
            z=dict(x)
            z["rank_score"],z["rank_detail"]=_tr_score_row(z,models)
            scored.append(z)
        scored.sort(key=lambda z:(z["rank_score"],z.get("return_pct",0)),reverse=True)
        for i,z in enumerate(scored,1):
            z["day_rank"]=i
            z["day_count"]=len(scored)
            ranked.append(z)
    return sorted(ranked,key=lambda x:(x["signal_date"],x["day_rank"],x["code"]))

def _tr_summary(rows):
    return _vg_summary(rows)

def _tr_rank_summary(ranked,rank_no,min_group=2):
    q=[x for x in ranked if int(x.get("day_count",0))>=min_group and int(x.get("day_rank",99))==rank_no]
    return _vg_summary(q),q

def _tr_all_multi_summary(ranked,min_group=2):
    q=[x for x in ranked if int(x.get("day_count",0))>=min_group]
    return _vg_summary(q),q

def _tr_group_stats(ranked):
    groups=_tr_group_by_day(ranked)
    multi=[a for a in groups.values() if len(a)>=2]
    triple=[a for a in groups.values() if len(a)>=3]
    return {
        "days_total":len(groups),
        "days_multi":len(multi),
        "days_triple":len(triple),
        "signals_multi":sum(len(a) for a in multi),
        "max_same_day":max([len(a) for a in groups.values()] or [0]),
    }

def _tr_validate(train_rows,valid_rows,blind_rows):
    models=_tr_train_feature_models(train_rows)
    if len(models)<3:
        return {"ok":False,"error":"TRAIN 특징 모델 부족"}

    train_ranked=_tr_rank_rows(train_rows,models)
    valid_ranked=_tr_rank_rows(valid_rows,models)
    blind_ranked=_tr_rank_rows(blind_rows,models)

    # 랭킹 실험은 "동일 날짜에 2개 이상 BASE 신호가 나온 경우"만 공정 비교.
    tr_all,tr_all_rows=_tr_all_multi_summary(train_ranked,2)
    va_all,va_all_rows=_tr_all_multi_summary(valid_ranked,2)
    bl_all,bl_all_rows=_tr_all_multi_summary(blind_ranked,2)

    tr1,tr1_rows=_tr_rank_summary(train_ranked,1,2)
    va1,va1_rows=_tr_rank_summary(valid_ranked,1,2)
    bl1,bl1_rows=_tr_rank_summary(blind_ranked,1,2)

    tr2,tr2_rows=_tr_rank_summary(train_ranked,2,2)
    va2,va2_rows=_tr_rank_summary(valid_ranked,2,2)
    bl2,bl2_rows=_tr_rank_summary(blind_ranked,2,2)

    tr3,tr3_rows=_tr_rank_summary(train_ranked,3,3)
    va3,va3_rows=_tr_rank_summary(valid_ranked,3,3)
    bl3,bl3_rows=_tr_rank_summary(blind_ranked,3,3)

    valid_delta=round(va1["win_rate"]-va_all["win_rate"],1) if va_all["n"] else 0.0
    blind_delta=round(bl1["win_rate"]-bl_all["win_rate"],1) if bl_all["n"] else 0.0

    stats={
        "train":_tr_group_stats(train_ranked),
        "valid":_tr_group_stats(valid_ranked),
        "blind":_tr_group_stats(blind_ranked),
    }

    # 랭킹은 신호를 삭제하는 필터가 아니므로 '월건수'는 기존 BASE 그대로.
    # PASS는 TOP1이 multi-signal 전체보다 재현성 있게 우월한지로 판정.
    checks={
        "VALID 복수신호일 5일 이상":stats["valid"]["days_multi"]>=5,
        "BLIND 복수신호일 5일 이상":stats["blind"]["days_multi"]>=5,
        "VALID TOP1 개선 ≥5%p":valid_delta>=5.0,
        "BLIND TOP1 개선 ≥5%p":blind_delta>=5.0,
        "BLIND TOP1 표본 5건 이상":bl1["n"]>=5,
        "BLIND TOP1 A손절 악화 없음":bl1["stop_rate"]<=bl_all["stop_rate"]+0.1 if bl_all["n"] else False,
    }
    status="PASS" if all(checks.values()) else "REJECT"
    ...
    return {
        ...
        "blind_ranked":blind_ranked,
        "stats":stats,
        "train":{"all_multi":tr_all,"top1":tr1,"top2":tr2,"top3":tr3},
        "valid":{"all_multi":va_all,"top1":va1,"top2":va2,"top3":va3},
        "blind":{"all_multi":bl_all,"top1":bl1,"top2":bl2,"top3":bl3},
        "valid_delta":valid_delta,
        "blind_delta":blind_delta,
        "checks":checks,
        "status":status,
        "reason":(
            "같은 날 여러 BASE 신호가 뜰 때 TOP1 우선순위가 VALID/BLIND에서 재현됨"
            if status=="PASS"
            else "TOP1 우선순위가 VALID/BLIND에서 충분히 재현되지 않음"
        ),
    }
