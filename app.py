...
APP_TITLE = "🧭 스톡 컴퍼스 V94-2"
APP_SUBTITLE = "경규님 전용 개인용 AI 투자비서 · 현재파일 기준 안정화"
...
st.set_page_config(page_title="스톡 컴퍼스 V94-2", page_icon="🧭", layout="centered")
...
def css():
    ...
    /* V94-2 전역 가독성 안정화 */
    .hold,.eval,.scorebox,.card,div[data-testid="stExpander"]{
        background:#ffffff!important;
        color:#0f172a!important;
        -webkit-text-fill-color:#0f172a!important;
    }
    ...
    .hero,.hero *,.action,.action *,.nav,.nav *,.thermo-marker,.thermo-marker *{
...
# V94-2 안정화: 매수타이밍 엔진 누락 함수 안전 보강
def position_52w(price, low_52w=None, high_52w=None):
    try:
        p = float(price or 0)
        lo = float(low_52w or 0)
        hi = float(high_52w or 0)
        if p <= 0 or lo <= 0 or hi <= lo:
            return None
        return max(0, min(100, (p - lo) / (hi - lo) * 100))
    except Exception:
        return None

def fetch_market_data(name):
    try:
        p, src = fetch_price(name)
    except Exception:
        p, src = None, "기본값"
    return {
        "name": norm(name),
...
def lifecycle_engine(name, price=None, result=None):
    ...
    if sec == "미국지수":
        ...
        score += 8
        stage = "🟢 성장 초중기"
        reasons.append("전력/자동화 테마는 성장성 관점에서 관심을 유지할 만합니다.")
    elif sec == "반도체":
        score += 4
        stage = "🟡 성장"
        reasons.append("반도체는 성장성은 있으나 변동성과 과열 여부 확인이 필요합니다.")
    elif sec == "디스플레이":
        score -= 4
        stage = "🟠 회복 확인"
        reasons.append("디스플레이는 업황 회복 확인이 필요합니다.")
    else:
        stage = "🟡 중립"
        reasons.append("섹터 기준으로 보수적으로 판단합니다.")

    if pos52 is not None:
        try:
            if pos52 <= 35:
                score += 8
                reasons.append("52주 위치가 낮아 추격 부담이 낮습니다.")
            elif pos52 >= 80:
                score -= 12
                reasons.append("52주 위치가 높아 선반영 위험이 있습니다.")
        except Exception:
            pass
    ...
    if cr is not None:
        try:
            ...
            if cr >= 7:
                score -= 8
                reasons.append("당일 급등으로 추격매수는 신중합니다.")
            elif cr <= -5:
                score -= 4
                reasons.append("당일 하락폭이 커서 추가 확인이 필요합니다.")
        except Exception:
            pass

    score = max(0, min(100, int(score)))
    rumor_risk = max(20, min(85, 100 - score + 25))
    if score >= 70:
        entry = "🟢 분할매수 가능"
    elif score >= 55:
        entry = "🟡 보유/소액"
