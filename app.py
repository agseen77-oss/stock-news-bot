...
# V125-1: Fake Bottom Alert / True Bottom Candidate
# 목적: 진바닥처럼 보이는 자리에서 가짜 바닥 위험을 먼저 경고합니다.
# 원칙: 자동매매/자동가중치 변경 없음. 현재 보유종목 위험 경고와 판단 보조만 수행합니다.
@st.cache_data(ttl=600, show_spinner=False)
def naver_daily_chart_v1251(name, count=180):
    """네이버 일봉 XML을 사용해 가짜바닥 판단용 OHLCV를 가져옵니다."""
    name = norm(name)
    code = code_map().get(name)
    if not code:
        return []
    try:
        ...
        txt = requests.get(url, headers={"User-Agent":"Mozilla/5.0"}, timeout=5).text
...
def _ma_slope(vals, n, back=5):
    if len(vals) < n + back:
        return 0
    now = sum(vals[-n:]) / n
    prev = sum(vals[-n-back:-back]) / n
    if prev == 0:
        return 0
    return (now / prev - 1) * 100


def _range_pct(rows, n=10):
    if len(rows) < n:
        return 999
    part = rows[-n:]
    hi = max(x['high'] for x in part)
    lo = min(x['low'] for x in part)
    close = part[-1]['close'] or 1
    return (hi / lo - 1) * 100 if lo else 999


def _volume_profile_support_v1251(rows, price, bucket_count=18):
    """일봉 종가×거래량 기반 근사 매물대. 실제 호가별 매물대는 아님."""
    if not rows or price <= 0:
        return None
    part = rows[-120:] if len(rows) > 120 else rows
    lows = [x['low'] for x in part if x.get('low')]
    highs = [x['high'] for x in part if x.get('high')]
    ...
    buckets = [0.0 for _ in range(bucket_count + 1)]
    centers = []
    for i in range(bucket_count + 1):
        centers.append(lo + step * i)
    for r in part:
        c = r.get('close', 0)
        v = r.get('volume', 0)
        if c <= 0 or v <= 0:
            continue
        idx = int((c - lo) / step) if step else 0
        idx = max(0, min(bucket_count, idx))
        buckets[idx] += v
    candidates = []
    for i, vol in enumerate(buckets):
        center = centers[i]
        if center <= price:
            dist = (price / center - 1) * 100 if center else 999
            if 0 <= dist <= 8:
                candidates.append((vol, center, dist))
    if not candidates:
        return None
    vol, center, dist = sorted(candidates, key=lambda x: x[0], reverse=True)[0]
    total = sum(buckets) or 1
    return {'price': center, 'distance_pct': dist, 'strength_pct': vol / total * 100}


def fake_bottom_analysis_v1251(name):
    rows = naver_daily_chart_v1251(name, 180)
    name = norm(name)
    if len(rows) < 70:
        return {
            'name': name, 'ok': False, 'message': '일봉 데이터 부족',
            'fake_risk': 50, 'true_score': 0, 'decision': '⚪ 확인불가', 'reasons': ['일봉 데이터가 부족해 진바닥/가짜바닥 판단을 보류합니다.']
        }
    ...
    prev = rows[-2]
    close = last['close']
    change = (close / prev['close'] - 1) * 100 if prev.get('close') else 0
    avg_vol20 = sum(vols[-21:-1]) / min(20, len(vols)-1) if len(vols) > 21 else (sum(vols[:-1]) / max(1, len(vols)-1))
    vol_ratio = last['volume'] / avg_vol20 * 100 if avg_vol20 else 0
    ma20 = _ma(closes, 20)
    ma60 = _ma(closes, 60)
    slope20 = _ma_slope(closes, 20, 5)
    slope60 = _ma_slope(closes, 60, 5)
    recent_low20 = min(x['low'] for x in rows[-20:])
    prev_low = min(x['low'] for x in rows[-80:-20])
    broke_prev_low = close < prev_low * 0.99
    near_prev_low = abs(close / prev_low - 1) * 100 <= 5 if prev_low else False
    range10 = _range_pct(rows, 10)
    support = _volume_profile_support_v1251(rows, close)
    support_near = bool(support and support.get('distance_pct', 999) <= 5 and support.get('strength_pct', 0) >= 4)
    ...
    if change <= -5:
        fake += 25; reasons.append(f'당일 급락 {change:+.2f}%: 떨어지는 칼 위험')
    elif change <= -3:
        fake += 15; reasons.append(f'당일 하락 {change:+.2f}%: 바닥 확인 전 진입 주의')
    else:
        true += 8; reasons.append(f'당일 급락 아님 {change:+.2f}%')

    if broke_prev_low:
        fake += 25; reasons.append('전저점 이탈: 가짜 바닥 핵심 위험')
    elif near_prev_low:
        true += 22; reasons.append('전저점 부근을 지키는 중: 진바닥 후보')
    else:
        true += 8; reasons.append('전저점 이탈 없음')

    if slope20 < -1:
        fake += 15; reasons.append(f'20일선 기울기 하락 {slope20:+.2f}%')
    elif slope20 > 0:
        true += 14; reasons.append(f'20일선 하락 멈춤/상승 {slope20:+.2f}%')

    if slope60 < -1:
        fake += 13; reasons.append(f'60일선 기울기 하락 {slope60:+.2f}%')
    elif slope60 > 0:
        true += 12; reasons.append(f'60일선 안정/상승 {slope60:+.2f}%')

    if support_near:
        true += 18; reasons.append(f'근사 매물대 지지 {support["distance_pct"]:.1f}% 위')
    else:
        fake += 8; reasons.append('근처 강한 매물대 지지 확인 약함')

    if range10 <= 12:
        true += 12; reasons.append(f'최근 10일 변동폭 {range10:.1f}%: 횡보/진정 신호')
    else:
        fake += 8; reasons.append(f'최근 10일 변동폭 {range10:.1f}%: 변동성 큼')

    if green and vol_ratio >= 130:
        true += 14; reasons.append(f'양봉 + 거래량 증가 {vol_ratio:.0f}%: 반전 시도')
    elif (not green) and vol_ratio >= 150:
        fake += 10; reasons.append(f'음봉 + 거래량 급증 {vol_ratio:.0f}%: 투매/매도압력 주의')

    if ma20 and close > ma20:
        true += 8; reasons.append('현재가 20일선 위')
    elif ma20 and close < ma20:
        fake += 6; reasons.append('현재가 20일선 아래')

    fake = max(0, min(100, int(fake)))
    true = max(0, min(100, int(true)))
    if fake >= 65 and fake > true:
        decision = '🔴 가짜 바닥 위험'
        action = '신규매수/추가매수 금지 · 전저점 회복과 횡보 확인 전까지 대기'
    elif fake >= 50 and fake >= true:
        decision = '🟠 가짜 바닥 주의'
        action = '바닥처럼 보여도 확인 필요 · 분할매수도 보수적으로'
    elif true >= 65 and true > fake:
        decision = '🟢 진바닥 후보'
        action = '즉시 몰빵이 아니라 전저점 유지·거래량 반전 확인 후 분할 접근'
    else:
        decision = '🟡 판단 보류'
        action = '진바닥/가짜바닥 신호가 혼재 · 하루 더 확인'

    return {
        'name': name, 'ok': True, 'close': close, 'change': change, 'vol_ratio': vol_ratio,
        'ma20_slope': slope20, 'ma60_slope': slope60, 'prev_low': prev_low,
        'broke_prev_low': broke_prev_low, 'near_prev_low': near_prev_low,
        'range10': range10, 'support': support, 'fake_risk': fake, 'true_score': true,
        'decision': decision, 'action': action, 'reasons': reasons[:8]
    }


def fake_bottom_ranking_v1251(data):
    items = []
    for h in data.get('holdings', []):
        n = norm(h.get('name', ''))
        if not n:
            continue
        try:
            items.append(fake_bottom_analysis_v1251(n))
        except Exception as e:
            items.append({'name': n, 'ok': False, 'decision': '⚪ 오류', 'fake_risk': 50, 'true_score': 0, 'reasons': [str(e)]})
    return sorted(items, key=lambda x: x.get('fake_risk', 0), reverse=True)


def render_fake_bottom_alert_v1251(data, compact=True):
    items = fake_bottom_ranking_v1251(data)
    if not items:
        card('🚨 V125-1 Fake Bottom Alert', '보유종목이 없어 가짜 바닥 위험을 판단하지 못했습니다.')
        return
    top = items[0]
    reasons = '<br>'.join([f'• {x}' for x in top.get('reasons', [])[:5]])
    sub = '진바닥처럼 보이는 자리에서 가짜 바닥 위험을 먼저 경고합니다. 전저점 이탈·당일 급락·20/60일선 하락·매물대 지지를 함께 봅니다.'
    st.markdown(
        '<div class="db-card">'
        '<div class="db-title">🚨 V125-1 Fake Bottom Alert</div>'
        f'<div class="db-sub">{sub}</div>'
        f'<div class="db-action">최우선 경고: {top.get("name","-")} · {top.get("decision","-")}<br>'
        f'가짜바닥 위험 {top.get("fake_risk",0)}점 · 진바닥 점수 {top.get("true_score",0)}점<br>'
        f'행동: {top.get("action","확인 필요")}</div>'
        f'<div class="db-sub"><b>핵심 근거</b><br>{reasons}</div>'
        '</div>',
        unsafe_allow_html=True
    )
    if not compact:
        st.markdown('<div class="db-card"><div class="db-title">가짜 바닥 위험 전체 순위</div>', unsafe_allow_html=True)
        for x in items:
            support = x.get('support') or {}
            support_txt = f'매물대 {support.get("distance_pct",0):.1f}% 위' if support else '매물대 지지 약함'
            st.markdown(
                '<div class="db-row">'
                f'<div class="db-name">{x.get("name","-")} · {x.get("decision","-")}</div>'
                f'<div class="db-meta">가짜위험 {x.get("fake_risk",0)}점 · 진바닥 {x.get("true_score",0)}점 · {support_txt}<br>'
                f'전저점 이탈: {"예" if x.get("broke_prev_low") else "아니오"} · 20일선기울기 {x.get("ma20_slope",0):+.2f}% · 60일선기울기 {x.get("ma60_slope",0):+.2f}%<br>'
                f'행동: {x.get("action","확인 필요")}</div></div>',
                unsafe_allow_html=True
            )
        st.markdown('<div class="db-sub">※ 매물대는 실제 호가별 매물대가 아니라 일봉 종가×거래량 기반 근사값입니다. 자동매매/자동가중치 변경은 하지 않습니다.</div></div>', unsafe_allow_html=True)
